...
const NewsFeed: React.FC<NewsFeedProps> = ({ news, limit = 15 }) => {
  ...
  return (
    <div className="space-y-2">
      {items.map(item => (
        <div
          key={item.id}
          className={`border-l-2 ${categoryColors[item.category] || 'border-l-border'} bg-card/50 rounded-r-md px-3 py-2 animate-slide-up`}
        >
